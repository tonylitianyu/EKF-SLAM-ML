...
rigid2d::Vector2D rigid2d::DiffDrive::calculateWheelVelocity(const Twist2D& ts){
    ...
    double wheel_vel_x_L = -(D/r)*body_ang + (1.0/r)*body_vel_x;  //Using equation (1), body twist to left wheel velocity
    double wheel_vel_x_R = (D/r)*body_ang + (1.0/r)*body_vel_x;   //Using equation (2), body twist to right wheel velocity
...
rigid2d::Twist2D rigid2d::DiffDrive::getBodyTwistForUpdate(double left_angle, double right_angle){
    double D = wheel_b*0.5;
    double r = wheel_r;
    double ts_angle = (r/(2.0*D))*(right_angle-left_angle);   //Using equation (3), find the twist angle from wheel angle
    double ts_x = (r/2.0)*(right_angle+left_angle);           //Using equation (4), find the twist x velocity from wheel angle
